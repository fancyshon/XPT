...
from tools.datasets import build_transform
from tools.myloader import CustomImageFolder, CifarFS

from timm.utils import NativeScaler, get_state_dict, ModelEma

import tools.utils as utils
import protopformer
from tools.engine_proto import train_one_epoch, evaluate, few_shot_evaluation
...
def get_args_parser():
    ...
    choices=['CUB2011U', 'Car', 'Dogs',"MiniImageNet", "tiered_imagenet", "CIFAR-FS", "FC100"],
    ...
    # Fwe-shot learning parameters
    parser.add_argument('--n_way', type=int, default=5)
    parser.add_argument('--n_shot', type=int, default=5)
    parser.add_argument('--n_query', type=int, default=10)
    parser.add_argument('--n_validation_tasks', type=int, default=200)
    parser.add_argument("--use_prototype_mean", type=str2bool, default=False)
    
    parser.add_argument("--log_print_freq", type=int, default=30)
    parser.add_argument("--orth_coe", type=float, default=0.5)
...
'''
sh scripts/fsl_cub.sh deit_tiny_patch16_224 128 1
'''
...
def main(args):
    ...
    if args.data_set == 'CUB2011U':
        train_set = CUB(split="train", training=True, image_size=args.img_size, transform=build_transform(is_train=True, args=args))
        val_set = CUB(split="val", training=False, image_size=args.img_size)
        dataset_train, args.nb_classes = build_dataset(is_train=True, args=args)
    elif args.data_set == 'MiniImageNet':
        train_set = MiniImageNet(root='datasets/MiniImageNet', split="train", training=True, image_size=args.img_size, transform=build_transform(is_train=True, args=args))
        val_set = MiniImageNet(root='datasets/MiniImageNet', split="val", training=False, image_size=args.img_size)
        args.nb_classes = 100
    elif args.data_set == "tiered_imagenet":
        train_set = TieredImageNet(split="train", training=True, image_size=args.img_size, transform=build_transform(is_train=True, args=args))
        val_set = TieredImageNet(split="val", training=False, image_size=args.img_size)
        args.nb_classes = 608
    elif args.data_set == "CIFAR-FS":
        train_set = CifarFS(pickle_file='datasets/CIFAR-FS/FC100_train.pickle', train=True, transform=build_transform(is_train=True, args=args))
        val_set = CifarFS(pickle_file='datasets/CIFAR-FS/FC100_val.pickle', train=False, transform=build_transform(is_train=False, args=args))
        args.nb_classes = 100
        # args.img_size = 32
        # import pickle
        # t = pickle.load(open('datasets/CIFAR-FS/FC100_train.pickle', 'rb'), encoding='bytes')
        # print(t[b"labels"])
        # print(train_set[0][0].shape)
        # train_set2 = MiniImageNet(root='datasets/MiniImageNet', split="train", training=True, image_size=args.img_size, transform=build_transform(is_train=True, args=args))
        # print(train_set2[0][0].shape)
        
    elif args.data_set == "FC100":
        train_set = datasets.ImageFolder(root='datasets/FC100/FC100_Dataset/train/train', transform=build_transform(is_train=True, args=args))
        # val_set = datasets.ImageFolder(root='datasets/FC100/FC100_Dataset/val/val', transform=build_transform(is_train=False, args=args))
        val_set = CustomImageFolder(root='datasets/FC100/FC100_Dataset/val/val', transform=build_transform(is_train=False, args=args))
        args.nb_classes = 100
        # print(train_set[0])
    ...
    if args.eval:
        ...
        logger.info(f"Accuracy of the network on the {len(val_set)} test images: {test_stats['acc1']:.1f}%")
        return
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        __global_values__["it"] += len(train_loader)
        ...
        if (epoch + 1) % 10 == 0:
            test_stats = few_shot_evaluation(data_loader=val_loader, model=model, device=device, args=args)
            ...
            tb_writer.add_scalar("epoch/fsl_acc", test_stats["fsl_acc"], epoch)
            ...
            logger.info(f"Accuracy of the network on the {len(val_set)} test images: {test_stats['acc1']:.1f}%")
            logger.info(f"Fsl accuracy: {test_stats['fsl_acc'] * 100}%")
            # logger.info(f"Fsl loss: {test_stats['fsl_loss']}")
            if max_accuracy < test_stats["acc1"] or max_accuracy < test_stats["fsl_acc"]:   # save the best
                logger.info(f"Saving the best model with accuracy: {test_stats['fsl_acc'] * 100}%")
            ...
            max_accuracy = max(max_accuracy, test_stats["fsl_acc"])
            logger.info(f'Max fsl accuracy: {(max_accuracy * 100) :.2f}%')

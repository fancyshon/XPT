...
class PPNet(nn.Module):
    ...
    def get_orthogonal_loss(self, total_proto_act, cls_attn_rollout, original_fea_len, label):
        batch_size, original_fea_size = total_proto_act.shape[0], int(original_fea_len ** (1/2))
        proto_per_class = self.num_prototypes_per_class
        discrete_values = torch.FloatTensor([[x, y] for x in range(original_fea_size) for y in range(original_fea_size)]).cuda() # (196, 2)
        discrete_values = discrete_values[None, :, :].repeat(batch_size * proto_per_class, 1, 1)    # (B * 10, 196, 2)
        ...
        # Calculate the orthogonality of total_proto_act
        total_proto_act_transpose = total_proto_act.transpose(1, 2)  # (B, 81, 10)
        orthogonality_loss = torch.matmul(total_proto_act, total_proto_act_transpose)  # (B, 10, 10)
        orthogonality_loss = torch.norm(orthogonality_loss - torch.eye(proto_per_class).cuda(), dim=(1, 2)).mean()
        
        return orthogonality_loss
    
    def get_separation_and_cluster_loss(self, total_proto_act, original_fea_len, label):
        prototype_of_correct_class = torch.t(self.prototype_class_identity.cuda()[:,label])
        prototype_of_wrong_class = 1 - prototype_of_correct_class 
        ...
        topk_attention, _ = torch.topk(total_proto_act, k = 15, dim=-1)
        mean_attention = torch.mean(topk_attention, dim=-1)  # (B, 2000)
        
        # Calculate the separation loss
        separation_loss, _ = torch.max(mean_attention * prototype_of_wrong_class, dim=1)
        separation_loss = torch.mean(separation_loss)
        
        # Calculate the cluster loss
        cluster_loss, _ = torch.max(mean_attention * prototype_of_correct_class, dim=1)
        cluster_loss = torch.mean(cluster_loss)
        
        return separation_loss, cluster_loss

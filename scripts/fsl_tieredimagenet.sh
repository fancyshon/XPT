...
for data_set in tiered_imagenet;
do
    prototype_num=6080
    # prototype_num=3040
    data_path=datasets
    python fsl_main.py \
        ...
        --output_dir=$output_dir/$data_set/$n_ways-way$n_shots-shot/$seed-$lr-$opt-$weight_decay-$epochs-$ft \
        --model=$model \
    ...
    # torchrun main.py
    # Multi GPUs
    # sh script/fsl_tieredimagenet.sh deit_tiny_patch16_224 256 1